...
### Coded age variable to match how it was recoded later in this markdown file
capdat$age10yr[capdat$RIDAGEYR >= 20 & capdat$RIDAGEYR <= 29] <- "20-29"
capdat$age10yr[capdat$RIDAGEYR >= 30 & capdat$RIDAGEYR <=39] <- "30-39"
capdat$age10yr[capdat$RIDAGEYR >= 40 & capdat$RIDAGEYR <= 49] <- "40-49"
...
finalsetRecSmall <- finalsetRec %>% 
  select(ocp, eth, bmi, insur, education, health, poverty, percbmi,age10yr, mesbmi, calcBMIcat) 

# get included/excluded to compare
finalsetRecSmall$insample <- complete.cases(finalsetRecSmall)

# examine included vs. computed BMI for manuscript reviewer
...
label(finalsetRecSmall$age10yr)<-"Age (years)"
...
tableone::CreateTableOne(vars = c('age10yr','eth','mesbmi','education','health','poverty','percbmi'), 
                             strata = 'insample', 
...
descr::CrossTable(x = analyticData$mesbmi, y = analyticData$ocp, prop.r = FALSE,
                  prop.t = FALSE, prop.chisq = FALSE, chisq = TRUE,
                  sresid = TRUE)
# nonsignificant assoc p = .08
...
```{r}
# make table comparing included and excluded
tableone::CreateTableOne(vars = c('age10yr','eth','mesbmi','education','health','poverty'), 
                             strata = 'percbmi', 
                             data = analyticData)
```
...
adj <- multinom(percbmi ~ ocp + eth + insur + health + poverty + education + age10yr, 
                data = analyticData)
...
```{r}
# Model with measured BMI original NHANES variable (Table 3)
bmi3 <- multinom(percbmi ~ ocp + eth + health + insur + poverty + education + age10yr + mesbmi, 
                data = analyticData)
t(exp(coef(bmi3))) # odds ratios
exp(confint(bmi3))

# Model with computed BMI variable (for reviewer request, not included in paper)
bmi3comp <- multinom(percbmi ~ ocp + eth + health + insur + poverty + education + age10yr + calcBMIcat, 
                data = analyticData)
t(exp(coef(bmi3comp))) # odds ratios
exp(confint(bmi3comp))
```

# Exploratory analysis of OCP moderation between BMI and Perceived Weight

```{r}
# Model 
plain3 <- multinom(percbmi ~ ocp + eth + health + insur + poverty + education + age10yr + mesbmi,
                  data = analyticData)

################# INTERACTION MODELS #######################

# Model 
interaction3 <- multinom(percbmi ~ ocp*mesbmi + eth + health + insur + poverty + education + age10yr, 
                        data = analyticData)
...
# Stratified by normal weight (Table 4)
options(scipen = 000)
test1.3 <- multinom(percbmi ~ ocp + eth + health + poverty + insur + education + age10yr, 
                 subset = mesbmi == "Normal" , 
                 data = analyticData)

t(exp(coef(test1.3))) 
exp(confint(test1.3))

# Stratified by overweight obese (Table 5)
test2.3 <- multinom(percbmi ~ ocp + eth + health + poverty + insur + education + age10yr, 
                 subset = mesbmi == "Over/Obese" , 
                 data = analyticData)

t(exp(coef(test2.3))) 
exp(confint(test2.3))

# Stratified by underweight (not included)
test2.4 <- multinom(percbmi ~ ocp + eth + health + poverty + insur + education + age10yr, 
                 subset = mesbmi == "Under" , 
                 data = analyticData)
...
# estimate the BMI unadjusted model
mlogit.OcpMod <- mlogit(formula = percbmi ~ 0 | ocp + eth + health + poverty + insur + education + age10yr,
                           data = finalDataForMlogit)
summary(mlogit.OcpMod)

# estimate it with two outcome categories instead
mlogit.OcpModTwo <- mlogit(formula = percbmi ~ 0 | ocp + eth + health + poverty + insur + education + age10yr,
                           data = finalDataForMlogit,
                           alt.subset = c("Overweight", "Normal Weight"))
summary(mlogit.OcpModTwo)

hmftest(x = mlogit.OcpMod, z = mlogit.OcpModTwo)

# estimate the BMI adjusted model
mlogit.adjOcpMod <- mlogit(formula = percbmi ~ 0 | ocp + eth + health + poverty + insur + education + age10yr + mesbmi,
                           data = finalDataForMlogit)
summary(mlogit.adjOcpMod)

# estimate it with two outcome categories instead
mlogit.adjOcpModTwo <- mlogit(formula = percbmi ~ 0 | ocp + eth + health + poverty + insur + education + age10yr + mesbmi,
                           data = finalDataForMlogit,
                           alt.subset = c("Overweight", "Normal Weight"))
...
# stratified models with Normal bmi  
finalsetRecNorm <- analyticData %>% filter(mesbmi == "Normal")
finalDataForMlogitNorm <- mlogit.data(data = finalsetRecNorm,
                                  choice = "percbmi",
                                  shape = "wide")

mlogit.adjOcpModNorm <- mlogit(formula = percbmi ~ 0 | eth + health + poverty + insur + education + age10yr + ocp,
                           data = finalDataForMlogitNorm)
summary(mlogit.adjOcpModNorm)

# estimate it with two outcome categories instead
mlogit.adjOcpModNormTwo <- mlogit(formula = percbmi ~ 0 | eth + health + poverty + insur + education + age10yr + ocp,
                           data = finalDataForMlogitNorm,
                           alt.subset = c("Overweight", "Normal Weight"))
...
# stratified models overwt obese bmi
finalsetRecOvOb <- analyticData %>% filter(mesbmi == "Over/Obese")
finalDataForMlogitOvOb <- mlogit.data(data = finalsetRecOvOb,
                                  choice = "percbmi",
                                  shape = "wide")

mlogit.adjOcpModOvOb <- mlogit(formula = percbmi ~ 0 | eth + health + poverty + insur + education + age10yr + ocp,
                           data = finalDataForMlogitOvOb)
summary(mlogit.adjOcpModOvOb)

# estimate it with two outcome categories instead
mlogit.adjOcpModOvObTwo <- mlogit(formula = percbmi ~ 0 | eth + health + poverty + insur + education + age10yr + ocp,
                           data = finalDataForMlogitOvOb,
                           alt.subset = c("Overweight", "Normal Weight"))
...
# stratified models underweight bmi
finalsetRecUnder <- analyticData %>% filter(mesbmi == "Under")
finalDataForMlogitUnder <- mlogit.data(data = finalsetRecUnder,
                                  choice = "percbmi",
                                  shape = "wide")

mlogit.adjOcpModUnder <- mlogit(formula = percbmi ~ 0 | eth + health + poverty + insur + education + age10yr + ocp,
                           data = finalDataForMlogitUnder)
summary(mlogit.adjOcpModUnder)

# estimate it with two outcome categories instead
mlogit.adjOcpModUnderTwo <- mlogit(formula = percbmi ~ 0 | eth + health + poverty + insur + education + age10yr + ocp,
                           data = finalDataForMlogitUnder,
                           alt.subset = c("Overweight", "Normal Weight"))
summary(mlogit.adjOcpModUnderTwo)

hmftest(x = mlogit.adjOcpModUnder, z = mlogit.adjOcpModUnderTwo)
